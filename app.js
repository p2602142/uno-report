...
async function loadReports() {
  ...
  try {
    ...
    const dTargetSatang = daysCount ? Math.round(mTargetSatang / daysCount) : 0;
    ...
    const achievement = mTargetSatang ? (total / mTargetSatang) * 100 : 0;
    ...
    const insights = [
      `Achievement ของช่วงที่เลือกอยู่ที่ <strong>${achievement.toFixed(1)}%</strong> เทียบกับ Target ${money(mTargetSatang)}`,
...
/* =========================================================
   DATABASE RESET & SEED DATA SYSTEM (ADMIN CONSOLE)
========================================================= */

$("btn-db-reset")?.addEventListener("click", async () => {
  const confirm1 = confirm("⚠️ คำเตือน: คุณต้องการรีเซ็ตข้อมูลและสร้างข้อมูลตัวอย่างใหม่ใช่หรือไม่?");
  if (!confirm1) return;

  const confirm2 = confirm("🚨 ยืนยันอีกครั้ง! การดำเนินการนี้จะลบยอดขายเดิมในเดือนปัจจุบันทั้งหมด");
  if (!confirm2) return;
  ...
  try {
    ...
    // 1. Delete Existing Month Data
    const currentRows = await getMonthSales(y, m);
    for (const row of currentRows) {
      await deleteDoc(doc(db, "sales", row.date));
    }

    // 2. Seed Target
    const sampleTargetSatang = inputToSatang(250000);
    await setDoc(doc(db, "targets", `${y}-${monthStr}`), {
      monthlyTargetSatang: sampleTargetSatang,
      updatedAt: serverTimestamp(),
      updatedBy: "system-seed"
    });

    // 3. Seed Sales Rows
    for (let day = 1; day <= Math.min(now.getDate(), days); day++) {
      const dateStr = `${y}-${monthStr}-${String(day).padStart(2, "0")}`;
      const cash = Math.floor(Math.random() * 3000 + 2000) * 100;
      const creditCard = Math.floor(Math.random() * 2500 + 1000) * 100;
      const qrPayment = Math.floor(Math.random() * 2000 + 500) * 100;

      const totalSatang = cash + creditCard + qrPayment;

      await setDoc(doc(db, "sales", dateStr), {
        date: dateStr,
        totalSalesSatang: totalSatang,
        payments: {
          cash,
          creditCard,
          qrPayment,
        ...
        voidBill: Math.floor(Math.random() * 2),
    ...
    monthTargets[`${y}-${monthStr}`] = sampleTargetSatang;
    showToast("สร้างข้อมูลจำลองและรีเซ็ตระบบเรียบร้อยแล้ว", "success");
    setActivePage("page-dashboard");
  } catch (err) {
    showToast("เกิดข้อผิดพลาดในการรีเซ็ต: " + err.message, "error");
  } finally {
    showLoading(false);
  }
});
